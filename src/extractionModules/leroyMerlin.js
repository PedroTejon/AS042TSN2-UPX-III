  ...
  do {
    ...
    const [avaliacoes, quantiasAvaliacoes] = await axios.get(urlAvaliacoes, {
      ...
      for (const avaliacaoAtual of response['data']['Results']) {
        ...
        quantiasAvaliacoes[avaliacaoAtual['ProductStatistics']['ProductId']] = avaliacaoAtual['ProductStatistics']['ReviewStatistics']['TotalReviewCount'];
      }
    ...
    for (const anuncio of anuncios) {
      ...
      if ((await db.query(`SELECT * FROM anuncios WHERE url = '${url}'`, [], dbConn)).length == 0) {
        ...
        const categoriaOrig = anuncio['categoryPageId'].length != 0 ? anuncio['categoryPageId'].slice(-2, -1)[0].split(' > ').pop() : anuncio['parentName'];
        const categoria = mapCategorias[categoriaOrig];
        let preco = null;
        ...
        await db.query(`CALL insert_anun(?, ?, ?, ?, ?, ?, 2, ?, ?)`, [nome, avaliacao, preco, descricao, url, foto, categoria, qntdAvaliacoes], dbConn);
      }
    } 
